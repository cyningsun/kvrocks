...
static CODE_STATE *code_state(void) {
  ...
  if (!init_done) {
    ...
    pthread_rwlock_init(&THR_LOCK_init_settings, nullptr);
  ...
  if (!(cs_ptr = my_thread_var_dbug())) {
    debug_thread_init();
    // std::cout << "Thread not initialised" << std::endl;
...
static int DbugParse(CODE_STATE *cs, const char *control) {
  ...
  */
  ...
  if (stack == &init_settings) pthread_rwlock_wrlock(&THR_LOCK_init_settings);
  ...
  if (stack == &init_settings) pthread_rwlock_unlock(&THR_LOCK_init_settings);
...
void _db_enter_(const char *_func_, int func_len, const char *_file_,
  ...
  switch (DoTrace(cs)) {
    ...
    case DO_TRACE:
      if (TRACING) {
        if (!cs->locked) pthread_mutex_lock(&THR_LOCK_dbug);
        DoPrefix(cs, _line_);
        Indent(cs, cs->level);
...
static void DoPrefix(CODE_STATE *cs, uint _line_) {
  ...
  if (cs->stack->flags & FILE_ON){
    if (_line_ != 0 && (cs->stack->flags & LINE_ON)){
      std::string file(BaseName(cs->file) );
      std::string filename_line = file + ":"+std::to_string(_line_);
      (void)fprintf(cs->stack->out_file, "%-40s",filename_line.c_str());
    }else{
      (void)fprintf(cs->stack->out_file, "%-40s", BaseName(cs->file));
    }
  }    
...
static bool Writable(const char *pathname) {
  ...
  if (access(pathname, F_OK) == 0) {
    if (access(pathname, W_OK) == 0) granted = true;
  } else {
...
bool debug_thread_init() {
  struct st_my_thread_var *tmp;

  if (mysys_thread_var()) return false;

  tmp = (struct st_my_thread_var *)calloc(1, sizeof(*tmp));
  if (!tmp) return false;
