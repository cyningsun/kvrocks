...
struct RocksCachelibOptions {
  ...
  // If true, enable Cachelib FB303 stats (not supported in open source version)
  bool fb303Stats = false;

  // An oncall name for FB303 stats (not supported in open source version)
  std::string oncallName;
};

// Type aliases for CacheLib types
using FbCache = facebook::cachelib::LruAllocator;
...
// static_cast (cache.get())->Erase(key);
class RocksCachelibWrapper : public rocksdb::SecondaryCache {
 public:
  RocksCachelibWrapper(std::unique_ptr<FbCache>&& cache, facebook::cachelib::PoolId pool)
  ...
  rocksdb::Status Insert(const rocksdb::Slice& key, void* value,
                         const rocksdb::Cache::CacheItemHelper* helper, bool force_erase) override;

  rocksdb::Status InsertSaved(const rocksdb::Slice& /*key*/, const rocksdb::Slice& /*saved*/,
                              rocksdb::CompressionType /*type*/, rocksdb::CacheTier /*source*/) override {
  ...
  void WaitAll(std::vector<rocksdb::SecondaryCacheResultHandle*> handles) override;
  ...
  // policy.
  // Note: In the open source version without Facebook internal utilities,
